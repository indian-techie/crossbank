...
import 'package:crossbank/model/qr/qr_pay_response.dart';
...
class ApiClient {
  final Dio _dio = Dio();
  Future<LoginResponseModel> login(LoginReqModel reqModel) async {
    try {
      var formData = FormData.fromMap({
        'mpin':reqModel.mpin,
        'username':reqModel.username,
        'agent_email':reqModel.agent_email,
        'ip':reqModel.ip,
        'device_token':reqModel.device_token,
        'device_type':reqModel.device_type,
      });
      _dio.options.baseUrl=ApiConstants.base_url;
      _dio.interceptors.add(AwesomeDioInterceptor(logRequestTimeout: false,
  ...
  Future<QR_PAYRES> payQr(Map<String, dynamic> payMap) async {
    try {
      _dio.options.baseUrl=ApiConstants.base_url;
      _dio.interceptors.add(AwesomeDioInterceptor(logRequestTimeout: false,
        logRequestHeaders: false,
        logResponseHeaders: false,
        logger: debugPrint,));
      Response response = await _dio.post(ApiConstants.pay_now_qr, data:payMap);
      // print(response.data);
      // print(response.headers);
      // print(response.requestOptions);
      // print(response.statusCode);

      return QR_PAYRES.fromJson(response.data);
    }on DioError catch (e) {
      print(e);
      return e.response!.data;
  ...
  Future<QR_PAYRES> payNumber(Map<String, dynamic> payMap) async {
    try {
      _dio.options.baseUrl=ApiConstants.base_url;
      _dio.interceptors.add(AwesomeDioInterceptor(logRequestTimeout: false,
        logRequestHeaders: false,
        logResponseHeaders: false,
        logger: debugPrint,));
      Response response = await _dio.post(ApiConstants.pay_now_number, data:payMap);
      // print(response.data);
      // print(response.headers);
      // print(response.requestOptions);
      // print(response.statusCode);
